...
(defn cartesian->spherical
  "Converts a map of Cartesian coordinates :x, :y and :z to spherical coordinates
  :r, :theta, and :phi."
  [c]
  (let [r (magnitude c)]
    {:r     r
...
(def g "Accelaration of gravity in meters/s^2" -9.8)
...
(defn scale
  "Mulitplies a map of x, y, and z coordinates by the given factor."
  [factor coordinates]
  (map-values (partial * factor) coordinates))

(defn acceleration
  "Total acceleration of the craft"
  [craft]
  (let [m (mass craft)]
...
(defn crashed?
  "Does this trajectory crash into the surface berfore 100 hours are up?"
  [trajectory]
...
(defn crash-time
  "Given a trajectory, returns the time a rocket impacted the ground."
  [trajectory]
...
(defn rejection
  "The component of coordinate map a *not* in the direction of coordinate map b."
  [a b]
...
(defn orientation
  ...
  (cond
    ...
    ; to say, stright up, away from the earth.
    ...
    ; Otherwise, just point stright ahead.
    :else (:velocity craft)))
